...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Set up IPC listener for hostname
    window.electronAPI.ipcRenderer.on('hostname', function(e, hostname) {
        console.log('Received hostname:', hostname);
        var maquinaInput = document.getElementById('maquina');
        if (!maquinaInput.value) {
            maquinaInput.value = hostname;
        }
    });
    
    // Set up IPC listener for local IP
    window.electronAPI.ipcRenderer.on('localIP', function(e, localIP) {
        console.log('Received local IP:', localIP);
        var servidorInput = document.getElementById('servidor');
        if (!servidorInput.value) {
            servidorInput.value = localIP + ':3000';
        }
    });
    
    // Set default machine name to hostname if not already set
    var maquinaInput = document.getElementById('maquina');
    if (!maquinaInput.value) {
        // Get hostname from the main process
        window.electronAPI.ipcRenderer.send('getHostname');
    }
    
    // Set default server address to local IP if not already set
    var servidorInput = document.getElementById('servidor');
    if (!servidorInput.value) {
        // Get local IP from the main process
        window.electronAPI.ipcRenderer.send('getLocalIP');
    }
